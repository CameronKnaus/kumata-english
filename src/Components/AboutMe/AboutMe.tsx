  ...
  return (
    <>
      <div
        ...
        onClick={() => navigate(PAGE_ROUTES.landingPage)}
      />
      <div className={styles.pageContainer}>
        <div className={`${styles.modalHeader} large-header-text`}>
          <div className={styles.introText}>{ABOUT_ME.sectionTitle}</div>
          <button
        ...
        <div className={styles.modalContainer}>
          ...
          {ABOUT_ME.textGroups.map((textGroup, index) => (
            <div key={`text-group-${index}`}>
              <h2
                className={`${index === 0 ? 'large-header-text' : 'header-text'} ${HEADER_STYLES[index]}`}
              >
                {textGroup.header}
              </h2>
              {textGroup.paragraphs.map((paragraph, paragraphIndex) => (
                <p
                  key={`text-group-${index}-paragraph-${paragraphIndex}`}
                  className="body-text"
                >
                  {paragraph}
                </p>
        ...
        <div
          className={`${styles.closingStatement} ${styles.closingLineTwo} ${closingFont}`}
        >
