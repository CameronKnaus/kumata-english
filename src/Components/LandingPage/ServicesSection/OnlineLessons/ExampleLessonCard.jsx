...
export default function ExampleLessonCard({
  ...
  return (
    <div className={styles.lessonTile}>
      <h3 className={styles.lessonTileTitle}>{title}</h3>
      <div className={styles.activityGroup}>
        {activityList.map((activityGroup, index) => (
          <div
            key={`${title}-activity-${index}`}
            className={styles.activityWrapper}
          >
            <div className={styles.timeFrame}>{activityGroup.timeFrame}</div>
            {Array.isArray(activityGroup.activity) ? (
              activityGroup.activity.map((item, index) => (
                <div
                  key={`${title}-activity-list-${index}`}
